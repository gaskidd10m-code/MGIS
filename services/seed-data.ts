...
const worldNewsArticles = [
    {
        ...
        excerpt: "Upon taking office in January 2025, President Trump's administration swiftly withdrew from the Paris Agreement and WHO, imposed broad tariffs, and prioritized 'America First' policies, ending an era of U.S.-led multilateralism.",
...
export async function seedDatabase() {
    try {
        ...
        for (const cat of categories) {
            const created = await firestoreService.createCategory(cat.name);
        ...
        console.log(`- ${worldNewsArticles.length} articles created`);
    } catch (error) {
